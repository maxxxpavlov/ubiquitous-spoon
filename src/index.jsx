...
function App(props) {
  ...
  const changeToSlide = (idx) => {
    if(slider.status !== 'openning' && idx < slides.length){
      setSliderState({
        idx: slider.idx, nextPage: idx, status: 'closing'
      })
    }
  }
  const moveToNext = () => changeToSlide(slider.idx + 1)
  const CurrentSlideComponent = slides[slider.idx]
  const currentSlide = <CurrentSlideComponent idx={slider.idx} toRight={slider.idx > slider.nextPage} status={slider.status} moveToNext={moveToNext}
   onOpen={(hisIdx) =>  setSliderState({idx: slider.idx, status: STATUS.open, nextPage: slider.nextPage})}>Slide 1</CurrentSlideComponent>
  let nextSlide
  if(slider.nextPage !== null && slider.status === STATUS.closing){
    const NextSlide = slides[slider.nextPage]
    nextSlide = <NextSlide idx={slider.nextPage} status="openning" moveToNext={moveToNext} onOpen={(hisIdx) => hisIdx === slider.nextPage && setSliderState({idx: slider.nextPage, status: STATUS.open, nextPage: null})}></NextSlide>
  }
  return (
    <div>
      <nav className="left-arrow">
        <ol>
        {slides.map((slideElement, i) => (<li key={i}>
          <div title={"Страница " + (i + 1)} aria-enabled={slider.status === STATUS.open} onClick={i !== slider.idx ? () => changeToSlide(i) : ""} className={"page" + ((slider.nextPage === null && i === slider.idx) || (slider.nextPage !== null && i === slider.nextPage) ? " current" : "")}></div>
          </li>))}
        </ol>
      </nav>
      <div class="slide">
